...
class BestFit(strategy.Strategy):
    def calculate(self, servers, job):
        ...
        # Servers data format: servers = [...,[server_type, server_type_id,
        # server_state, server_avail_time, server_cores, server_mem,
        # server_disk_space],...,]

        # According to pseudocode: set bestFit and MinAvail to large number
        # e.g. MAX_INT.
        bestFit = 99999999
        bestFitServer = False
        minAvail = 99999999
        ...
        # ServerTypes as defined by system.xml:
        # "tiny","small","medium","large","xlarge" -> map to serverType IDs ->
        # 0, 1, 2, 3, 4, 5
        sysxml = [0, 1, 2, 3, 4, 5]
        ...
        # If server is idle -> availTime = serverType[boottime] +
        # job[submit_time].
        ...
        for serverType in sysxml:
            for server in servers:
                ...
                if fitnessVal < bestFit and server[3] == 3:
                    initBestFit = server

                # if fitness value is less than bestfit or fitness value is
                # same as bestFit and server available time is less than
                # minAvail
                if fitnessVal < bestFit or (
                        fitnessVal == bestFit and server[3] < minAvail):
                    bestFit = fitnessVal
                    bestFitServer = server
                    minAvail = server[3]

        # This if-else will look like: if bestFit : return bestFit : else :
        # return bestFit Active server based on initial resource capacity.
        if bestFitServer:
